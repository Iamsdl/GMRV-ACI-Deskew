...
cv::Mat Hough::ApplyTransform(const cv::Mat& points, float& angle, float& confidence, int threshold, int offset,bool draw)
{
	cv::Mat dst;
	cv::cvtColor(points, dst, cv::COLOR_GRAY2BGR);
	// Standard Hough Line Transform
	std::vector<cv::Vec3f> lines; // will hold the results of the detection
	HoughLines(points, lines, 1, STEP_ANGLE * CV_PI / 180, threshold, 0, 0, (offset + MIN_ANGLE - STEP_ANGLE) * CV_PI / 180, (offset + MAX_ANGLE + STEP_ANGLE) * CV_PI / 180); // runs the actual detection
	...
	for(size_t i = 0; i < lines.size(); i++)
	{
		if (lines[i][2] > lines[0][2] / HOUGH_CONFIDENCE_MOD)
		{
			float rho = lines[i][0], theta = drawAngle;
			cv::Point pt1, pt2;
